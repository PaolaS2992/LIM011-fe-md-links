...
const getPathMd = pathAbsolute => (0, _app.verifyDirectory)(pathAbsolute).then(directory => {
  let newArray = []; // Array de los path.

  if (directory === true) {
    return (0, _app.readDirectory)(pathAbsolute).then(files => {
      const arrayPromise = []; // Array de Promesas.

      files.forEach(file => {
        const newPath = (0, _app.unionPath)(pathAbsolute, file);
...
}).catch(err => err); // 2. Convertir de Markdown a HTML.


exports.getPathMd = getPathMd;

const renderHtml = documentMd => (0, _app.readDocument)(documentMd).then(result => {
  const documentHtml = (0, _app.converterHtml)(result);
  return documentHtml;
}).catch(err => err); // 3. Devuelve un ARRAY DE OBJETOS con los LINKS.
...
const getMdLink = path => {
  ...
  return getPathMd(pathAbsolute).then(arrMd => {
    let newArray = [];
    const newMap = arrMd.map(docMd => renderHtml(docMd).then(docHtml => arrLink(docHtml, pathAbsolute)));
...
const arrLinkValidate = (docHtml, pathAbsolute) => {
  ...
  let newArray = [];
  const arrObjValidate = arrObj.map(element => (0, _app.statusHttp)(element.href).then(resHttp => {
    const obj = {
...
const getMdLinkValidate = path => {
  ...
  return getPathMd(pathAbsolute).then(arrMd => {
    let newArray = [];
    const newMap = arrMd.map(docMd => renderHtml(docMd).then(docHtml => arrLinkValidate(docHtml, pathAbsolute)));
...
const mdLinks = (path, options) => options.validate ? getMdLinkValidate(path) : getMdLink(path); // 9. Opcion stats
